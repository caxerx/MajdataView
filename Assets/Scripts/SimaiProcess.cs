...
namespace MajdataEdit
{
    static class SimaiProcess
    {
        ...
        /// </summary>
        /// <param name="data">data string of maidata.txt</param>
        /// <returns>if the read process faced any error</returns>
        static public bool ReadData(string data)
        {
            int i = 0;
            other_commands = "";
            try
            {
                string[] maidataTxt = data.Split("\n");
            ...
            {
                Console.WriteLine("读取谱面时出现错误: 在maidata.txt第" + (i + 1) + "行:\n" + e.Message);
                return false;
            }
        }
